...
- (void)setupAllBtns
{
    for (int i = 0; i < ZCKeyboardBtnCount; i++) {
        ...
        if (i == 9) {  // 确定按钮
            ...
            btn.titleLabel.font = [UIFont systemFontOfSize:KScreenWidth * 0.046875];
            [btn addTarget:self action:@selector(okBtnClick) forControlEvents:UIControlEventTouchUpInside];
        } else if (i == 10) {  // 0 按钮
            [btn setTitle:@"0" forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:KScreenWidth * 0.06875];
            btn.tag = 0;
...
- (void)layoutSubviews
{
    ...
    // 定义间距
    CGFloat pad = KScreenWidth * 0.015625;
    
    // 定义x y w h
    CGFloat x;
    CGFloat y;
    CGFloat w = KScreenWidth * 0.3125;
    CGFloat h = KScreenWidth * 0.14375;
...
- (void)deleteBtnClick
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:TradeKeyboardDeleteButtonClick object:self];
}
...
- (void)okBtnClick
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:TradeKeyboardOkButtonClick object:self];
}
...
- (void)numBtnClick:(UIButton *)numBtn
{
    ...
    userInfo[TradeKeyboardNumberKey] = @(numBtn.tag);
    [[NSNotificationCenter defaultCenter] postNotificationName:TradeKeyboardNumberButtonClick object:self userInfo:userInfo];
}
